...
- [ ] **P1-25** Credential prompts and agents (§7 Credentials): `credential-helper` binary gains askpass mode and is set as `GIT_ASKPASS`, `SSH_ASKPASS` (+`SSH_ASKPASS_REQUIRE=force`) on every git spawn unless the user configured `core.sshCommand`/`GIT_SSH`; prompts are forwarded to the running app over a local socket (named pipe on Windows) and rendered as native dialogs: passphrase (with Remember in keychain + `ssh-add` on success), HTTPS username/token (with Remember + "Sign in with GitHub/GitLab instead" link), unknown-host-key Accept/Reject with fingerprint; changed host keys are never auto-accepted. Agent detection: `SSH_AUTH_SOCK`, Windows OpenSSH agent pipe, Pageant; warn when no agent is running. Never write to `~/.gitconfig`, `~/.ssh/config` or `known_hosts`. Tests: fake `ssh`/`git` scripts drive each prompt kind; passphrase stored and replayed from an in-memory keyring; custom `core.sshCommand` leaves env untouched. · model: opus
...
- [ ] **P1-26** Settings → Credentials: list keychain entries per host (HTTPS tokens, passphrases) with Remove; Settings → SSH keys: list `~/.ssh` keys with agent-loaded status, Copy public key, Add to agent, Test connection (`ssh -T git@host` with output). · model: sonnet
